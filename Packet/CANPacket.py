...
class CANPacketField:
    def __init__(self, name: str, address: str, type: str,
            frequency: int, bit_range: str, unit: str = '', value: str = ''):
...
class CANPacket:
    ...
    def add_field(self, row):
        ...
        frequency = int(row[3][:-2]) if len(self.fields) == 0 else self.fields[0].frequency
        self.fields.append(CANPacketField(row[4], address, row[5], frequency, row[6], row[7], row[8]))
